// GF-822 — loadEvals: the callable fetch worker that both mount and retry() invoke.
// Symmetric with useRuns.test.js; tested directly (vitest node env, no jsdom / RTL).
import { describe, it, expect, vi, afterEach } from 'vitest';
import { loadEvals } from './useEvals';
...
function setters() {
  return { setEvals: vi.fn(), setError: vi.fn(), setLoading: vi.fn() };
}

afterEach(() => { vi.unstubAllGlobals(); });

describe('loadEvals (GF-822 retry worker)', () => {
  it('success → sets evals and clears the error (recovery)', async () => {
    stubStorage('tok');
    vi.stubGlobal('fetch', okFetch({ evals: [{ id: 'e1' }] }));
    const s = setters();

    await loadEvals(s);

    expect(s.setLoading).toHaveBeenCalledWith(true);
    expect(s.setEvals).toHaveBeenCalledWith([{ id: 'e1' }]);
  ...
  it('non-401 error → sets the error message, does not clear it', async () => {
    ...
    expect(s.setError).toHaveBeenCalledWith('API 500: /evals');
  ...
  it('GF-829 — pre-aborted signal: finally guard skips setLoading(false) (no post-unmount setState)', async () => {
    stubStorage('tok');
    vi.stubGlobal('fetch', okFetch({ evals: [] }));
    const s = setters();
    const ctrl = new AbortController();
    ctrl.abort();

    await loadEvals({ ...s, signal: ctrl.signal });
